...
#' @description A multi-class classifier under the Linear Discriminant Analysis (LDA) framework.
#'
...
#' @param num_threads number of cores to use for parallel computations. Default is 3.
#' @export
solda.fit <- function(y_data, x_data.train, x_data.test, lambda1="default", lambda2=0.25, weights = "default",
                      num_folds = 10, tolerance = 0.0000001, max_iter = 100000, num_threads = 3){
  
  ## compute the nuber of classes in the data
  Nclasses <- length(unique(y_data))
